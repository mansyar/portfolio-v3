...
│   │   └── generated/            # Build-time outputs (gitignored except articles-content.json)
│   │       └── articles-content.json  # Pre-compiled article metadata + HTML (generated by compile-articles.mjs)
│   └── styles/
│       └── xp-theme.css          # XP design tokens (see §5)
├── scripts/
│   └── compile-articles.mjs  # Build-time MDX → HTML compilation
├── astro.config.mjs
└── package.json
```
...
graph LR
    ...
    D --> E[Compile articles to JSON]
    E --> F[astro build]
    E -->     G[Deploy to Cloudflare Pages]
